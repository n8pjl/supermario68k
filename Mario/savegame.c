...
//	Temp = Levelsetfile_get_pointer_and_lock();
	...
	for(C=0;C<3;C++){
		...
		if( (C!=Slot) && (Levelsetdata.Savegames[C]!=0) ){
			...
			Size += Size%2;
			memcpy(TempBuffer/*Fg_plane.p.big_vscreen*/+Size, HeapDeref (Temp)+2+Levelsetdata.Savegames[C],Levelsetdata.Savegame_size[C]);
			//update saveslot pointer
			((levelsetdata*)TempBuffer/*Fg_plane.p.big_vscreen*/)->Savegames[C] = Levelsetdata.Savegames[C] = Size;
...
//	HeapUnlock(	Levelsetfile_sym->handle );
	...
	((levelsetdata*)TempBuffer/*Fg_plane.p.big_vscreen*/)->Savegames[(short)Slot] = Levelsetdata.Savegames[(short)Slot] = Size;
	...
	memcpy(TempBuffer/*Fg_plane.p.big_vscreen*/+Size,&SavePlayer,sizeof(saveplayer));
	...
	*(TempBuffer/*Fg_plane.p.big_vscreen*/+Size + 9) = Levelsetdata.CurrentWorld;
...
//	Size += 10;
	...
	short MapSize = Map_data.Height*Map_data.Width+sizeof(map_trigger)*Map_data.Nr_of_triggers+sizeof(map_object)*Map_data.Nr_of_objects ;
	
	char* TempBuffer2 = Fg_mask.p.big_vscreen;
	memcpy(TempBuffer2/*Fg_mask.p.big_vscreen*/,Map,MapSize);//copy possibly modified map with objects and triggers
	
	short OldPCX=SavePlayer.PrevCompX;short OldPCY=SavePlayer.PrevCompY;short OldMapX=SavePlayer.MapX;short OldMapY=SavePlayer.MapY;
	short OldL = SavePlayer.L;
	short OldFgX = FgX;short OldFgY = FgY;
	
	//New: V 1.01 Added check whether or not warp zone is to be loaded. Bugfix
	Load_map((Levelsetdata.CurrentWorld==Levelsetdata.Commonfile?Commonfilename:Levelfilename));//load original map
	//End of new
	...
	SavePlayer.PrevCompX=OldPCX;SavePlayer.PrevCompY=OldPCY;
	SavePlayer.MapX=OldMapX;SavePlayer.MapY=OldMapY;
	SavePlayer.L = OldL;
	...
	short RLESize = RLECompress(TempBuffer/*Fg_plane.p.big_vscreen*/+Size, TempBuffer2/*Fg_mask.p.big_vscreen*/, MapSize);
	...
	((levelsetdata*)TempBuffer/*Fg_plane.p.big_vscreen*/)->Savegame_size[(short)Slot] = Levelsetdata.Savegame_size[(short)Slot] = Slotsize;
	//Levelsetdata.Savegame_size[(short)Slot] = Slotsize;
	//((levelsetdata*)Fg_plane.p.big_vscreen)->Savegame_size[(short)Slot] = Size-((levelsetdata*)Fg_plane.p.big_vscreen)->Savegames[(short)Slot];
	//((levelsetdata*)Fg_plane.p.big_vscreen)->Savegame_size[(short)Slot] = Levelsetdata.Savegame_size[(short)Slot] = SlotSize;
	...
	//New: V 1.01 Modified the file saving to automaticly archive save game file, doing a garbage collection if neccessarry
	short Tries = 0;
	
	while(Tries<2){//0 or 1 try
		
		if( EM_findEmptySlot(Size+100) ){//large enough empty slot in archive mem?
			//save:
			//Unarchive
			...
			if(FWrite (TempBuffer/*Fg_plane.p.big_vscreen*/, Size, &fsPtr)!=FS_OK){//write everythiong to file
				goto failure;
			}
			//close file
			FClose (&fsPtr);
			...
			if(EM_GC (FALSE)){
				Load_gfx_from_file();//Retrieve pointers to GFX files, might be invalid after garbage collection
			};
			Tries++;
...
3. If step 2 has been performed 2 times unsuccessfully, it is not possible to archive.
    If step 2 has been performed successfully, the required archive is available. Unarchive the file,
    write to it and archive it
*/
...
/*Autoarchive stuff
	...
	while(Tries<2){//0 or 1 try
		...
		if( EM_findEmptySlot() ){//large enough empty slot in archive mem?
			//archive
...
*/
	...
	failure://save failed
		...
		//DrawGrayStrExt2B(20+screen_offset_sg_x,40+screen_offset_sg_y,"Saving failed!",A_REPLACE|A_SHADOWED,F_8x10,dBufHPL_G,dBufHPD_G);
		DrawString(20+screen_offset_sg_x,40+screen_offset_sg_y,"Saving failed!",A_REPLACE|A_SHADOWED,F_8x10);
...
//	Raw0 += sizeof(levelsetdata)+21 + 9*(Levelsetdata.Nr_of_files+1);
	//char* Raw = Raw0 + sizeof(levelsetdata)+21 + 9*(Levelsetdata.Nr_of_files+1);
//	char* Raw = Raw0;
	char *Raw, *Raw0;
	//Raw += Raw%2;
...
//	Temp = Levelsetfile_get_pointer_and_lock();
	...
	//New: V 1.01 Added check whether or not warp zone is to be loaded. Bugfix
	Load_map((Levelsetdata.CurrentWorld==Levelsetdata.Commonfile?Commonfilename:Levelfilename)/*Levelfilename*/);//load map
	//End of new
	...
	//New: V 1.01 Fixed bug where mario is not immortal after loading a game, even though he used a star just before saving
...
//		(map_object*)Map_objects[C].Handler = (Map_objects[C].Mode==30?Handle_boat:Handle_ship);
...
HANDLE File_get_pointer_and_lock(const char* FileName/*,SYM_ENTRY* File_sym*/){
	...
	if( !(File_sym = SymFindPtr (SYMSTR(FileName), 0)) )//flags = 4 will change when the documentation of VAT.h is updated
