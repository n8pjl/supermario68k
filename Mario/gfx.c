...
//  memcpy (&GameTextData, HeapDeref (Temp)+2, sizeof(gametextdata));
  memcpy (&GameTextData, Texts - sizeof(gametextdata), sizeof(gametextdata));//same as above, but a little smaller
	...
	Fg_mask.p.sprites = Fg_plane.p.sprites + 32*/*Tilefiledata.*/Nr_of_fg_tiles*sizeof(unsigned short);

	Bg_plane.sprites = Fg_mask.p.sprites + 16*/*Tilefiledata.*/Nr_of_tilemasks*sizeof(unsigned short);

	Fg_plane.tabanim = Bg_plane.sprites + 32*/*Tilefiledata.*/Nr_of_bg_tiles*sizeof(unsigned short);//Tilemasks + 16*Gfxfiledata.Nr_of_tilemasks;

	Fg_mask.tabanim = Fg_plane.tabanim + /*Tilefiledata.*/Nr_of_fg_animations*4*sizeof(unsigned short);

	Map_plane.p.sprites = Fg_mask.tabanim + /*Tilefiledata.*/Nr_of_fg_animations*4*sizeof(unsigned short);
		
	Map_plane.tabanim = Map_plane.p.sprites + 32*/*Tilefiledata.*/Nr_of_map_tiles*sizeof(unsigned short);
		
	//New: V 1.01 Changed "ma_sprts" to "mario\\ma_sprts"
	if( !(Temp = Spritefile_sym_h = File_get_pointer_and_lock("mario\\ma_sprts")) ){
		return 2;//error "failed to open gfx file"
	}
	...
	Smallsprites = (char*)(Enemysprites + /*Spritefiledata.*/Size_of_enemysprites);
  ...
  Games = (char*)(Boss_sprites + /*Spritefiledata.*/Size_of_boss_sprites);
	...
	if( !(/*Temp = */Bg_file_sym_h = File_get_pointer_and_lock("mario\\ma_bckgr")) ){
		return 2;//error "failed to open gfx file"
	}
