...
inline void Handlebosses(){
	...
	if( TestCollide162h_R(Player.X,Player.Y,BossG->X,BossG->Y,Player.Height,BossG->Height,Mariosprites+2*Marioanimtab[SavePlayer.Spritebase][SavePlayer.Spritenr],BossG->Sprite) || TestCollide162h_R(Player.X,Player.Y,BossG->X+16,BossG->Y,Player.Height,BossG->Height,Mariosprites+2*Marioanimtab[SavePlayer.Spritebase][SavePlayer.Spritenr],BossG->Sprite+3*BossG->Height2) ){
		
		if( BossG->Active && Player.IsFalling && (Player.Y < BossG->Y) ){//something more: x match (hit head)
...
short Boss_free_down(boss *Boss){
	...
	return min(Free_down(Boss->X,Boss->Y+Boss->Height),Free_down(Boss->X+15,Boss->Y+Boss->Height));
	...
	if(Boss->Active){
		...
		if(Boss->Mode){
			...
			if( (/*Boss->*/Face>0) ){//dir = right
				...
				if(/*Boss->*/Face<=Free_right(Boss->X+15,&Boss->Y,Boss->Height,/*Boss->*/Face)){
					Boss->X += /*Boss->*/Face;
				}	
				else{
					/*Boss->*/Face = -/*Boss->*/Face;
				...
				if((-/*Boss->*/Face)<=Free_left(Boss->X,&Boss->Y,Boss->Height,/*Boss->*/Face)){
					Boss->X += /*Boss->*/Face;
				}
				else{
					/*Boss->*/Face = -/*Boss->*/Face;
			...
			if( !(Boss->Life%2) ){//jumping
				...
				if(abs(Boss->X-Player.X)>=64){
					...
					/*Boss->*/Data0 = abs(Boss->X-Player.X)/4;
			...
			*/		
					...
					Boss->Y -= (Temp<4?Temp:4);//Boss->Data1;
		...
		if(Boss->Life){
			...
			if( (abs(Boss->X-Player.X)<64) && (abs(Boss->Y-Player.Y)<64) && !(/*Boss->*/Data0) ){
		...
		if(Boss->Mode){
			...
			if(Boss->Life){
				...
				if( (++Boss->Data1)==80 ){
					...
					Add_boss_shot(Boss->X+(/*Boss->*/Face>0?24:7),Boss->Y+7,(/*Boss->*/Face>0?2:-2), Temp2/*((Player.Y-Boss->Y)>0?2:-2)*/ );
				}
				else{
				...
				Temp = Boss_free_down(Boss);//Free_down(Boss->X+8,Boss->Y+Boss->Height);
				...
				if(abs(xDist)<threshold){
					...
					//jump against player
					if( (!/*Boss->*/Data0  && !Temp/*Free_down(Boss->X+8,Boss->Y+Boss->Height)*/) ){
				...
				if(/*Boss->*/Face>0){//right
					if( Free_right(Boss->X+15,&Boss->Y,Boss->Height,/*Boss->*/Face) ){
						Boss->X += /*Boss->*/Face;
					}
					else{
						if(( Free_right(Boss->X+15,&Boss->Y,Boss->Height-16,/*Boss->*/Face) && !Temp/*Free_down(Boss->X+8,Boss->Y+Boss->Height)*/ )){
							//jump against player
							/*Boss->*/Data0 = 16;
						};
					};
					...
					if(Free_left(Boss->X,&Boss->Y,Boss->Height,/*Boss->*/Face)){
						Boss->X += /*Boss->*/Face;
					}
					else{
						if( (Free_left(Boss->X,&Boss->Y,Boss->Height-16,/*Boss->*/Face) && !Temp/*Free_down(Boss->X+8,Boss->Y+Boss->Height)*/) ){
							//jump against player
							/*Boss->*/Data0 = 16;
						};
					};
...
//	void* dp = GrayDBufGetActivePlane (DARK_PLANE); 
	...
	FilledRectEraseLight(47+screen_offset_sg_x,7,115+screen_offset_sg_x,38);
	//FastFilledRect_Erase_R(dBufHPD/*_G*/,47+screen_offset_sg_x,7,115+screen_offset_sg_x,38);
	FilledRectEraseDark(47+screen_offset_sg_x,7,115+screen_offset_sg_x,38);
	
	//DrawGrayStrExt2B(50+screen_offset_sg_x,10,Texts+GameTextData.Ending1,A_REPLACE,F_4x6,dBufHPD/*_G*/,dBufHPL/*_G*/);//"THANK YOU MARIO!"
	//DrawGrayStrExt2B(50+screen_offset_sg_x,20,Texts+GameTextData.Ending2,A_REPLACE,F_4x6,dBufHPD/*_G*/,dBufHPL/*_G*/);//"YOU ARE MY HERO"
	DrawString(50+screen_offset_sg_x,10,Texts+GameTextData.Ending1,A_REPLACE,F_4x6);
	DrawString(50+screen_offset_sg_x,20,Texts+GameTextData.Ending2,A_REPLACE,F_4x6);
			
	if(Object->Data0){
		//DrawGrayStrExt2B(60+screen_offset_sg_x,30,Texts+GameTextData.Ending3,A_REPLACE,F_4x6,dBufHPD/*_G*/,dBufHPL/*_G*/);//"*KISS*"
		DrawString(60+screen_offset_sg_x,30,Texts+GameTextData.Ending3,A_REPLACE,F_4x6);
	};
}
	...
	if( Boss->Active && (Boss->Mode==0) ){
		...
		if(!Boss->Data0){//not in a jump
			...
			switch(Boss->State){
				...
				case 91:
					...
					Boss->Data0 = abs(Player.X - Boss->X+16);///2;
				...
				case 181:
					//fire
					Add_big_fireball_shot(Boss->X+((Player.X>Boss->X)?28:-4),Boss->Y+8,((Player.X>Boss->X)?4:-4),0);
				break;
		...
		if(Boss->Data0>0){//Handle jump and powerjump
			...
			*/
			...
			if(Boss->Data0==0)
				Boss->Sprite = (Boss->X>Player.X?bowser_run_left_spr:bowser_run_right_spr);
			
			short /*Dist=16,Dist2,*/DX;
		
//			for(DX=0;DX<=16;DX+=16){
//				Dist2 = Free_down(Boss->X+DX,Boss->Y+Boss->Height);
...
//					Dist = Dist2;
				...
				/*if( Dist<=3 ){
					...
					if(Boss->Data1 == 1){//powerjump
						...
						unsigned char Tile = Get_tile(Boss->X+DX,Boss->Y+Boss->Height+5);
							
						if( (Tile>=solid_interactive_bounch_low) && (Tile<=solid_interactive_bounch_high) ){
...
//				while(1);
				...
				if(!Boss_free_down(Boss)/*Free_down(Boss->X+DX,Boss->Y+Boss->Height)*/){//hit the floor?
					...
					//break bricks if any
						...
						Boss->Data1 = 0;//end power jump
						
					}
					Boss->Data0 = 0;//Bang;//end jump or bounche
				}
		...
		//add something here: Jump up if trapped in hole: will happend after first powerjump landing
	
		if(Boss->Face<0){//left
			
			
			if(Free_left(Boss->X,&Boss->Y,Boss->Height,-2)>=abs(Boss->Face)){
				Boss->X += Boss->Face;
			}
			else{
				...
				for(DY=16;DY<=48;DY+=16){
					Y = Boss->Y-DY;
					if( /*(Dist==0) &&*/ (Free_left(Boss->X,&Y,Boss->Height,-2)>=abs(Boss->Face)) ){
						Boss->Data0 = DY;//while(1);
						//Boss->Y = Y+DY;
						break;
					}
				}
		...
		if(Boss->Face>0){//right
			...
			if( /*(Dist==0) &&*/ (Free_right(Boss->X+16,&Boss->Y,Boss->Height,2)>=abs(Boss->Face)) ){
				Boss->X += Boss->Face;
			}
			else{
				//check if bowser can jump up
				...
				for(DY=16;DY<=48;DY+=16){
					Y = Boss->Y-DY;
					if(Free_right(Boss->X+16,&Y,Boss->Height,2)>=abs(Boss->Face)){
						Boss->Data0 = DY;//while(1);
						//Boss->Y = Y+DY;
						break;
					}					
...
//			DX = min(Free_down(Boss->X,Boss->Y+Boss->Height),3);
//			DX = min(DX,Free_down(Boss->X+16,Boss->Y+Boss->Height));
			Boss->Y += DX;
			Boss->Mode--;
			Boss->Data0 = Boss->Data1 = 0;//abort jump/powerjump: Avoid breaking bricks when he shouldn't
			Boss->State = 170;//90;//might change: maybe it becomes to easy with 90 (powerjump immedeately after jumpkill)
...
/*
		...
		if(Boss->Life){
			...
			if( (abs(Boss->X-Player.X)<80) /*&& (abs(Boss->Y-Player.Y)<80)*/ && !(Boss->Data0) ){
				Boss->Data0 = 20;								
			};
...
/*			BossG->Sprite = bowser_angry;
			...
			BossG->X = FgX + screen_width/2-16;//put bowser in the middle of screen (x-wise)
			BossG->Y = FgY + screen_height;
...
//			Draw_ending_text(0);
			...
			for(;((Player.X<(7*16-8)) && (Items[0].X>(7*16+8)));Player.X++){//mario and princess running against each other
				Items[0].X--;
				Render();
//				Draw_ending_text(1);
...
void Boss_die_1(boss *Boss){//Boss_die_1 and Boss_die_2 merged together for size optimization.
...
void Add_big_fireball_shot(short X,short Y, char DirX, char DirY){
	...
	for(C=0;C<nr_of_enemyshots;C++){
		if(!(Enemyshots[C].Mode)){
			...
			Enemyshots[C].Sprite = ( DirX>0 ? big_fireball_right_spr:big_fireball_left_spr );//temp
			Enemyshots[C].Mode = 1;
			break;
		}
	};*/
