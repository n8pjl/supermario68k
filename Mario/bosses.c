...
inline void Handlebosses() {
  ...
  if (TestCollide162h_R(
          Player.X, Player.Y, BossG->X, BossG->Y, Player.Height, BossG->Height,
          Mariosprites +
              2 * Marioanimtab[SavePlayer.Spritebase][SavePlayer.Spritenr],
          BossG->Sprite) ||
      TestCollide162h_R(
          Player.X, Player.Y, BossG->X + 16, BossG->Y, Player.Height,
          BossG->Height,
          Mariosprites +
              2 * Marioanimtab[SavePlayer.Spritebase][SavePlayer.Spritenr],
    ...
    if (BossG->Active && Player.IsFalling &&
        (Player.Y < BossG->Y)) { // something more: x match (hit head)
...
void Add_killed_boss(boss *Boss, short Type) {
  ...
  Items[0].Active = Type;
  Items[0].Sprite =
      (Type == 1 ? boss1_killed_spr : magic_wand_spr); // magic_wand_spr;
  ...
  if (Boss->Active) {
    ...
    if (Boss->Mode) {
      ...
      if ((/*Boss->*/ Face > 0)) { // dir = right
        ...
        if (/*Boss->*/ Face <=
            Free_right(Boss->X + 15, &Boss->Y, Boss->Height, /*Boss->*/ Face)) {
          Boss->X += /*Boss->*/ Face;
        } else {
          /*Boss->*/ Face = -/*Boss->*/ Face;
        };
        ...
        if ((-/*Boss->*/ Face) <=
            Free_left(Boss->X, &Boss->Y, Boss->Height, /*Boss->*/ Face)) {
          Boss->X += /*Boss->*/ Face;
        } else {
          /*Boss->*/ Face = -/*Boss->*/ Face;
        };
    ...
    if (Boss->Life) {
      ...
      if ((abs(Boss->X - Player.X) < 64) && (abs(Boss->Y - Player.Y) < 64) &&
          !(/*Boss->*/ Data0)) {
    ...
    if (Boss->Mode) {
      ...
      if (Boss->Life) {
        ...
        if ((++Boss->Data1) == 80) {
          ...
          Add_boss_shot(Boss->X + (/*Boss->*/ Face > 0 ? 24 : 7), Boss->Y + 7,
                        (/*Boss->*/ Face > 0 ? 2 : -2),
        ...
        Temp =
            Boss_free_down(Boss); // Free_down(Boss->X+8,Boss->Y+Boss->Height);
        ...
        if (/*Boss->*/ Face > 0) { // right
          if (Free_right(Boss->X + 15, &Boss->Y, Boss->Height,
                         /*Boss->*/ Face)) {
            Boss->X += /*Boss->*/ Face;
          } else {
            if ((Free_right(Boss->X + 15, &Boss->Y, Boss->Height - 16,
                            /*Boss->*/ Face) &&
                 !Temp /*Free_down(Boss->X+8,Boss->Y+Boss->Height)*/)) {
              // jump against player
              /*Boss->*/ Data0 = 16;
          ...
          if (Free_left(Boss->X, &Boss->Y, Boss->Height, /*Boss->*/ Face)) {
            ...
            if ((Free_left(Boss->X, &Boss->Y, Boss->Height - 16,
                           /*Boss->*/ Face) &&
                 !Temp /*Free_down(Boss->X+8,Boss->Y+Boss->Height)*/)) {
              // jump against player
              /*Boss->*/ Data0 = 16;
...
void Draw_ending_text(object *Object) {
  ...
  FilledRectEraseLight(47 + screen_offset_sg_x, 7, 115 + screen_offset_sg_x,
                       38);
  ...
  if (Object->Data0) {
    ...
    DrawString(60 + screen_offset_sg_x, 30, Texts + GameTextData.Ending3,
               A_REPLACE, F_4x6);
  };
}
...
void Bowser_handler(boss *Boss1) {
  ...
  if (Boss->Active && (Boss->Mode == 0)) {
    ...
    if (!Boss->Data0) { // not in a jump
      ...
      case 181:
        // fire
        Add_big_fireball_shot(Boss->X + ((Player.X > Boss->X) ? 28 : -4),
                              Boss->Y + 8, ((Player.X > Boss->X) ? 4 : -4), 0);
        break;
      }
    }
          ...
          if (Boss->Data1 == 1) { // powerjump
            ...
            for (DX = 0; DX <= 32; DX += 16) {
              ...
              if ((Tile >= solid_interactive_bounch_low) &&
                  (Tile <= solid_interactive_bounch_high)) {
                // trigger interactive tile (Likely brick)
                Interactive_tile_handlers[Tile - solid_interactive_low](
                    Boss->X + DX, Boss->Y + Boss->Height + 5);
                // Bang = 16;
              }
            }
      ...
      if (Free_left(Boss->X, &Boss->Y, Boss->Height, -2) >= abs(Boss->Face)) {
        ...
        if( !Boss_free_down(Boss)/*Free_down(Boss->X,Boss->Y+Boss->Height) && !Free_down(Boss->X+16,Boss->Y+Boss->Height)*/ ){
          for (DY = 16; DY <= 48; DY += 16) {
            Y = Boss->Y - DY;
            if (/*(Dist==0) &&*/ (Free_left(Boss->X, &Y, Boss->Height, -2) >=
                                  abs(Boss->Face))) {
    ...
    if (Boss->Face > 0) { // right
      ...
      if (/*(Dist==0) &&*/ (Free_right(Boss->X + 16, &Boss->Y, Boss->Height,
                                       2) >= abs(Boss->Face))) {
        Boss->X += Boss->Face;
      } else {
        // check if bowser can jump up
        ...
        if( !Boss_free_down(Boss)/*Free_down(Boss->X,Boss->Y+Boss->Height) && !Free_down(Boss->X+16,Boss->Y+Boss->Height)*/ ){
          for (DY = 16; DY <= 48; DY += 16) {
            Y = Boss->Y - DY;
            if (Free_right(Boss->X + 16, &Y, Boss->Height, 2) >=
                abs(Boss->Face)) {
              Boss->Data0 = DY; // while(1);
              // Boss->Y = Y+DY;
              break;
    ...
    if (Boss->Mode) {
      ...
      Boss->State = 170; // 90;//might change: maybe it becomes to easy with 90
      ...
      if (Boss->Life) {
        ...
        if ((abs(Boss->X - Player.X) < 80) /*&& (abs(Boss->Y-Player.Y)<80)*/ &&
            !(Boss->Data0)) {
          Boss->Data0 = 20;
        };
        ...
        */
        ...
        for (; ((Player.X < (7 * 16 - 8)) && (Items[0].X > (7 * 16 + 8)));
