...
inline void Player_bounching_shell_hadler() {
  ...
  for (C = 0; C < max_nr_of_shells; C++) {
    ...
    if (/*Shells[C].*/ Shell->Active) { // test if Active(existing)
      ...
      if ((/*Shells[C].*/ Shell->X > (FgX - 64)) &&
          (/*Shells[C].*/ Shell->X < (FgX + BIG_VSCREEN_WIDTH + 64)) &&
          (/*Shells[C].*/ Shell->Y > (FgY - 64)) &&
          (/*Shells[C].*/ Shell->Y <
           (FgX + BIG_VSCREEN_HEIGHT + 64))) { // in active area ?

        if (/*Shells[C].*/ Shell->Dir > 0) { // dir = right ?

          // if(
          // abs(Shells[C].Dir)>Free_right(Shells[C].X,&Shells[C].Y,16,Shells[C].Dir)
          // ){//not free right
          Dist = Free_right(/*Shells[C].*/ Shell->X, &/*Shells[C].*/ Shell->Y,
                            16, /*Shells[C].*/ Shell->Dir);
          if (Dist <= 0) { // not free right

            Temp = Get_tile(/*Shells[C].*/ Shell->X + 15 +
                                /*Shells[C].*/ Shell->Dir,
                            /*Shells[C].*/ Shell->Y);
            if ((Temp >= solid_interactive_bounch_low) &&
              ...
              Interactive_tile_handlers[Temp - solid_interactive_low](
                  /*Shells[C].*/ Shell->X + 15 + /*Shells[C].*/ Shell->Dir,
                  /*Shells[C].*/ Shell->Y);
            };
          ...
          Dist = Free_left(/*Shells[C].*/ Shell->X, &/*Shells[C].*/ Shell->Y,
                           16, /*Shells[C].*/ Shell->Dir);
          // if(
          // abs(Shells[C].Dir)>Free_left(Shells[C].X,&Shells[C].Y,16,Shells[C].Dir)/*(Get_tile(Shells[C].X-Shells[C].Dir-3
          // ,Shells[C].Y)>=solid_low) &&
          // (Get_tile(Shells[C].X-Shells[C].Dir-3,Shells[C].Y+15)>=solid_low)*/
          // ){//not free left
          if (Dist <= 0) { // not free left

            Temp = Get_tile(/*Shells[C].*/ Shell->X +
                                /*Shells[C].*/ Shell->Dir - 3,
                            /*Shells[C].*/ Shell->Y);
            if ((Temp >= solid_interactive_bounch_low) &&
                (Temp <= solid_interactive_bounch_high)) {
              Interactive_tile_handlers[Temp - solid_interactive_low](
                  /*Shells[C].*/ Shell->X + /*Shells[C].*/ Shell->Dir - 3,
                  /*Shells[C].*/ Shell->Y);
            };
        ...
        (Get_tile(Shells[C].X+15,Shells[C].Y+16+15)>=solid_down_low) ){//free
        down ? Temp = (15-(Shells[C].Y+15)%16);//distance to next tile down
                }
                else{
                        Temp = 16;
        };*/
        Temp = Free_down(/*Shells[C]*/ Shell->X,
                         /*Shells[C].*/ Shell->Y + 16); // free down ?

        /*Shells[C].*/ Shell->Y +=
            (Temp > enemy_fallspeed) ? enemy_fallspeed : Temp; // fall
        ...
        // if( !(Shells[C].Count) &&
        // (TestCollide16(Player.X,Player.Y,Shells[C].X,Shells[C].Y,16,Mariosprites+2*Marioanimtab[Player.Spritebase][Player.Spritenr],shell_0_sprite)
        // /*&&*/||
        // TestCollide16(Player.X,Player.Y+Player.Height-16,Shells[C].X,Shells[C].Y,16,Player.Sprite+Player.Height-16,shell_0_sprite)
        // ) ){
        if (!(/*Shells[C].*/ Shell->Count) &&
            TestCollide162h_R(Player.X, Player.Y, /*Shells[C].*/ Shell->X,
                              /*Shells[C].*/ Shell->Y, Player.Height, 16,
                              Mariosprites +
                                  2 * Marioanimtab[SavePlayer.Spritebase]
                                                  [SavePlayer.Spritenr],
                              shell_0_sprite)) {
          if (SavePlayer.Attribs & 0b10000000) { // mario star?
            // New: V 1.04 Added missing animation when a shell is killed by
            // "Star" Mario
            ...
            if (Player.IsFalling &&
                (Player.Y <
                 /*Shells[C].*/ Shell->Y)) { // player jumped on shell

              // if( ((Player.X>=Shells[C].X) && (Shells[C].Dir>0)) ||
              // ((Player.X<=Shells[C].X) && (Shells[C].Dir<0)) ){

              /*Shells[C].*/ Shell->Active = 0;
              /*Shells[C].*/ Shell->Enemy->Attribs = 0b11100010;
              (/*Shells[C].*/ Shell->Enemy->Handler) = Enemy_handler_6;
              /*Shells[C].*/ Shell->Enemy->Life =
                  /*Shells[C].*/ Shell->Enemy->Mode;
              /*Shells[C].*/ Shell->Enemy->Mode = 60;
              /*Shells[C].*/ Shell->Enemy->X = /*Shells[C].*/ Shell->X;
              /*Shells[C].*/ Shell->Enemy->Y = /*Shells[C].*/ Shell->Y;

              /*if( (Shells[C].Enemy->Sprite == turtle_right_sprite) ||
              (Shells[C].Enemy->Sprite == turtle_left_sprite) ){
...
void Handle_collided_shells(object* Object){
        ...
        if( Object->Active > 28 )//with speed 4 in y dir this nr ensures that
none is removed before they have left the screen Object->Active =
0;//Enemy_remove(Enemy);
};*/
