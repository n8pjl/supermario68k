...
//	void *dBufHPL, *dBufHPD;
	...
	while(Playing){//!Keystate.Esc
		...
		*/
		...
		GrayClearScreen2B(/*GrayDBufGetHiddenPlane(LIGHT_PLANE)*/dBufHPL_G,/*GrayDBufGetHiddenPlane(DARK_PLANE)*/dBufHPD_G);
		...
		for(C=0;C<4;C++){
			...
			*/
			...
			DrawSprite16SMASKR(RollX_176(C*44+X1)+screen_offset_sg_x,30,12,game_mushrom_l+4*38*Sequence[C],game_mushrom_l+38+4*38*Sequence[C],blank_sprite);
			DrawSprite16SMASKR(RollX_176(C*44+X1+16)+screen_offset_sg_x,30,12,game_mushrom_r+4*38*Sequence[C],game_mushrom_r+38+4*38*Sequence[C],blank_sprite);			
			...
			DrawSprite16SMASKR(RollX_176(C*44+X2)+screen_offset_sg_x,42,16,game_mushrom_l+4*38*Sequence[C]+12,game_mushrom_l+38+4*38*Sequence[C]+12,blank_sprite);
			DrawSprite16SMASKR(RollX_176(C*44+X2+16)+screen_offset_sg_x,42,16,game_mushrom_r+4*38*Sequence[C]+12,game_mushrom_r+38+4*38*Sequence[C]+12,blank_sprite);
			...
			DrawSprite16SMASKR(RollX_176(C*44+X3)+screen_offset_sg_x,58,10,game_mushrom_l+4*38*Sequence[C]+28,game_mushrom_l+38+4*38*Sequence[C]+28,blank_sprite);
			DrawSprite16SMASKR(RollX_176(C*44+X3+16)+screen_offset_sg_x,58,10,game_mushrom_r+4*38*Sequence[C]+28,game_mushrom_r+38+4*38*Sequence[C]+28,blank_sprite);
		...
		GrayOutlineRect(41+screen_offset_sg_x,23,121+screen_offset_sg_x,77,COLOR_WHITE);
		GrayOutlineRect(40+screen_offset_sg_x,22,122+screen_offset_sg_x,78,COLOR_WHITE);
		
		if(State==5){//match
		
			if((AnimCount/16)%2){
			//if((AnimCount>>4)&0b00000001){//this optimization didn't improve anything :-(
				...
				DrawSprite16SMASKR(79+screen_offset_sg_x,44,15,up_sprite,up_sprite+15,blank_sprite);
				GrayClipSprite8_SMASK_R(67+screen_offset_sg_x,44,15,Smallsprites+Sprite,Smallsprites+Sprite+15,(unsigned char*)blank_sprite,/*GrayDBufGetHiddenPlane(LIGHT_PLANE)*/dBufHPL_G,/*GrayDBufGetHiddenPlane(DARK_PLANE)*/dBufHPD_G);
			};
		...
		GrayDBufToggleSync_SetPointers();//GrayDBufToggleSync();//set drawbuffer as active screen
		...
		if(Keystate.Run && !(Previous_keystate.Run) && !(Stopping) ){
			...
			switch(State){
				case 0:
					...
					break;
				case 1:
					//Stopping = (Find_stop_pos(X2) - X2) - 176;//denne virker nesten: noen få pixler for langt til høyre
				 	Stopping = -(176+(X2-Find_stop_pos(X2)));//(Find_stop_pos(X2) - X2) - 176;
...
void SetPixel(short X,short Y){//used in cardgame. Slower, but gives size optimization.
...
//	void *dBufHPL, *dBufHPD;
	...
	while(Miss<2){
		...
		*/
		...
		FastFilledRect_Draw_R(/*GrayDBufGetHiddenPlane(LIGHT_PLANE)*/dBufHPL_G,0,0,239,127);//,0,25,43,75);
		...
		GrayOutlineRect(2+screen_offset_sg_x+26*CX,1+screen_offset_sg_y+32*CY,27+screen_offset_sg_x+26*CX,34+screen_offset_sg_y+32*CY,COLOR_BLACK);
		GrayOutlineRect(3+screen_offset_sg_x+26*CX,2+screen_offset_sg_y+32*CY,26+screen_offset_sg_x+26*CX,33+screen_offset_sg_y+32*CY,COLOR_BLACK);
		...
		for(C=0;C<6;C++){
			for(D=0;D<3;D++){
				...
				GrayOutlineRect(4+screen_offset_sg_x+26*C,3+screen_offset_sg_y+32*D,23+screen_offset_sg_x+26*C,30+screen_offset_sg_y+32*D,COLOR_BLACK);
				GrayOutlineRect(24+screen_offset_sg_x+26*C,6+screen_offset_sg_y+32*D,25+screen_offset_sg_x+26*C,32+screen_offset_sg_y+32*D,COLOR_DARKGRAY);
				GrayOutlineRect(7+screen_offset_sg_x+26*C,31+screen_offset_sg_y+32*D,23+screen_offset_sg_x+26*C,32+screen_offset_sg_y+32*D,COLOR_DARKGRAY);			
...
//				EXT_SETPIX(/*GrayDBufGetHiddenPlane(LIGHT_PLANE)*/dBufHPL,5+26*C+screen_offset_sg_x,29+screen_offset_sg_y+32*D);EXT_SETPIX(/*GrayDBufGetHiddenPlane(DARK_PLANE)*/dBufHPD,5+screen_offset_sg_x+26*C,29+screen_offset_sg_y+32*D);
				...
				SetPixel(5+26*C+screen_offset_sg_x,4+screen_offset_sg_y+32*D);//same as above, but smaller
				SetPixel(22+26*C+screen_offset_sg_x,4+screen_offset_sg_y+32*D);
				SetPixel(22+26*C+screen_offset_sg_x,29+screen_offset_sg_y+32*D);
				SetPixel(5+26*C+screen_offset_sg_x,29+screen_offset_sg_y+32*D);
...
//					GrayClipSprite16_SMASK_R(6+screen_offset_sg_x+26*C,6+screen_offset_sg_y+11+32*D,11,card_top+11,card_top+22+11,blank_sprite,/*GrayDBufGetHiddenPlane(DARK_PLANE)*/dBufHPD,/*GrayDBufGetHiddenPlane(LIGHT_PLANE)*/dBufHPL);
					...
					GrayClipSprite16_SMASK_R(6+screen_offset_sg_x+26*C,6+screen_offset_sg_y+32*D,22,card_top,card_top+22,blank_sprite,dBufHPD_G,dBufHPL_G);
					...
					if( (!Flash) || ((Flash%2) || ( ((C!=PX) || (D!=PY)) && ((C!=CX) || (D!=CY)) ) ) ){
					
						if(CardGame[C+6*D]<=2){
							Sprite = (CardGame[C+6*D]==1?pu_mushrom_sprite:(star_anim));
							...
							DrawSprite16SMASKR(6+screen_offset_sg_x+26*C,9+screen_offset_sg_y+32*D,16,Sprite,Sprite+16,Sprite+32);
						}
						else{
							...
							switch(CardGame[C+6*D]){
								...
								case 3://flower
									Sprite += 63*32;//Sprite = (unsigned short*)Fg_plane.p.sprites + 63*32;
									break;
								case 4://Up
									Sprite += 180*32;;//Sprite = (unsigned short*)Fg_plane.p.sprites + 180*32;
									StringCopy(String,"1 UP");
									break;
								case 5://10 Coins
									Sprite += 4*32;//Sprite = (unsigned short*)Fg_plane.p.sprites + 4*32;
							...
							DrawString(8+screen_offset_sg_x+26*C,9+screen_offset_sg_y+15+32*D,String,A_NORMAL,F_4x6);
							//A_REPLACE|A_SHADOWED
						
							GrayClipISprite16_RPLC_R(6+screen_offset_sg_x+26*C,7+screen_offset_sg_y+32*D,16,Sprite,/*GrayDBufGetHiddenPlane(LIGHT_PLANE)*/dBufHPL_G,/*GrayDBufGetHiddenPlane(DARK_PLANE)*/dBufHPD_G);
		...
		if(Wait){
			...
			if( (--Wait)==0){
				if(Turn){//turn cards back after mismatch and wiev
					...
					if(C==18){//if all cards were checked, i.e. no unmatched cards on the table...
						Miss=2;//exit
				//		New_card_game();
					}
