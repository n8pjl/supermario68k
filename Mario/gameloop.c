...
/*		if(Keystate.Enter && !Previous_keystate.Enter)
	...
	//New: V 1.05: Maximum lives = 99. Prevents player from getting game over when lives exceeds 127
	...
	if(PrevLives>SavePlayer.Lives){
		SavePlayer.Lives = PrevLives-1;//Make sure you never loose more that 1 life at a time. There are some indications that it occurs rarely...
	}
	//end of New
...
void Gameloop(){
	...
	while( (!Exit) && (ErrorCode==0) && (SavePlayer.Lives>0) ){
		...
		for(C=0;C<10000;C++);//time delay, will probably do it different later...
...
/*	FastFilledRect_Erase_R(ActiveLight,32,32,128,68);
	...
	FastFilledRect_Erase_R(ActiveLight,screen_width/2-40,screen_height/2-20,screen_width/2+40,screen_height/2+20);
	FastFilledRect_Erase_R(ActiveDark,screen_width/2-40,screen_height/2-20,screen_width/2+40,screen_height/2+20);
//	GrayFastFillRect_R(ActiveLight,ActiveDark, screen_width/2-40,screen_height/2-20,screen_width/2+40,screen_height/2+20,COLOR_WHITE);
	GrayFastOutlineRect_R(ActiveLight,ActiveDark,screen_width/2-41,screen_height/2-21,screen_width/2+41,screen_height/2+21,COLOR_BLACK);
...
//	DrawGrayStrExt2B(36,36,String,A_REPLACE|A_SHADOWED,F_6x8,ActiveLight,ActiveDark);
	DrawGrayStrExt2B(screen_width/2-42/2,screen_height/2-(6+8),String,A_REPLACE|A_SHADOWED,F_6x8,ActiveLight,ActiveDark);
	...
	DrawGrayStrExt2B(screen_width/2-60/2,screen_height/2+6,String,A_REPLACE|A_SHADOWED,F_6x8,ActiveLight,ActiveDark);
...
//	HeapUnlock(	Levelsetfile_sym->handle );
	...
	do{
		...
		//memcpy( &Levelfilename, HeapDeref (Temp)+2+sizeof(levelsetdata)+21+9*Levelsetdata.CurrentWorld, 9 );
...
/*		if(Exit==100){//Warp Zone 
	...
	}while( (Levelsetdata.CurrentWorld < Levelsetdata.Nr_of_files) && (ErrorCode==0) && (Exit==0) );
