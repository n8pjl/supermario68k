    ...
    if (Enemies[Curr_enemy].Life) {
      ...
      if ((Enemies[Curr_enemy].Respawn < 0) ||
        ...
        if (!(Enemies[Curr_enemy].Jumping) &&
          ...
          C = Free_down(Enemies[Curr_enemy].X,
                        Enemies[Curr_enemy].Y + Enemies[Curr_enemy].Height);
          Enemies[Curr_enemy].Y +=
              ((C >= enemy_fallspeed) ? (enemy_fallspeed) : (C));

          if (!(Enemies[Curr_enemy].Attribs & 0b00000100) &&
              ((Enemies[Curr_enemy].Y + Enemies[Curr_enemy].Height) >
               (Leveldata.Height * 16)))
            Enemies[Curr_enemy].Life = 0;
        };

        if (TestCollide162h_R(Player.X, Player.Y, Enemies[Curr_enemy].X,
                              Enemies[Curr_enemy].Y, Player.Height,
          ...
          if (((SavePlayer.Attribs & 0b10000000) &&
            ...
            if ((Enemies[Curr_enemy].Attribs & 0b00000010) ||
                ((Enemies[Curr_enemy].Attribs & 0b10000000) &&
                 Player.IsFalling && (Player.Y < Enemies[Curr_enemy].Y))) {
        ...
        for (C = 0; C < nr_of_bounching_tiles; C++) {
          if (Bounching_tiles[C].Active) {
            if (BoundsCollide(Bounching_tiles[C].X, Bounching_tiles[C].Y, 16,
                              Enemies[Curr_enemy].X, Enemies[Curr_enemy].Y,
                              Enemies[Curr_enemy].Height)) {
              Enemies[Curr_enemy].Die(&Enemies[Curr_enemy]); // kill enemy;
            };
          };
        ...
        if (Player.Racoonspin) { // test if smashed by racoon suit tale
          ...
          if ((Enemies[Curr_enemy].Attribs & 0b00000001) &&
              BoundsCollide2(Player.X + (Player.Face == 1 ? -9 : 15),
                             Player.Y + 17, 6, 9, Enemies[Curr_enemy].X,
                             Enemies[Curr_enemy].Y, Enemies[Curr_enemy].Height,
                             16)) {
        ...
        for (C = 0; C < max_nr_of_shells; C++) {
          if (Shells[C].Active) {
            if (BoundsCollide(Shells[C].X, Shells[C].Y, 16,
                              Enemies[Curr_enemy].X, Enemies[Curr_enemy].Y,
                              Enemies[Curr_enemy].Height)) {
              if ((Enemies[Curr_enemy].Attribs & 0b00001000)) {
                Enemy_die(&Enemies[Curr_enemy]);
      ...
      if (Enemies[Curr_enemy].Respawn &&
          !((active_x_left < (Enemies[Curr_enemy].RespawnX * 16)) &&
            (active_x_right > (Enemies[Curr_enemy].RespawnX * 16)) &&
            (active_y_upper < (Enemies[Curr_enemy].RespawnY * 16)) &&
            (active_y_lower >
             (Enemies[Curr_enemy].RespawnY * 16)))) { // Respawn
        ...
        // D0D1);
        if ((Enemies[Curr_enemy].Respawn > 0) &&
            (Enemies[Curr_enemy].Respawn <= handler1_users_high)) {
          Generate_handler_1_enemy(Curr_enemy, Enemies[Curr_enemy].Respawn, -1,
...
void Enemy_handler_1(enemy *Enemy) { // walk in one dir,if not free walk to the
  ...
  if (Enemy->Active == 3) { //
    ...
    if (Enemy->Face > 0) {
      ...
      if (Get_tile(Enemy->X + 15, Enemy->Y + Enemy->Height + 15) <
          solid_down_low) {
      ...
      if (Enemy->Face < 0) {
        // if(Free_down(Enemy->X-15,Enemy->Y+Enemy->Height)<16){
        // if(Free_down(Enemy->X-16,Enemy->Y+Enemy->Height)){
        if (Get_tile(Enemy->X, Enemy->Y + Enemy->Height + 15) <
            solid_down_low) {
          Enemy->Face = -Enemy->Face;
          if (Enemy->Mode)
            Enemy->Sprite += Enemy->Height2 * 3;
...
void Enemy_handler_3(
  ...
  if (Player.X >= Enemy->X) {   // right
    if (Player.Y <= Enemy->Y) { // up
      ...
      if (/*Enemy->*/ Mode == 124)
        Enemy_add_fireball(Enemy->X + 12, Enemy->Y + (Enemy->Life > 0 ? 2 : 29),
                           enemy_fireball_speed, -enemy_fireball_speed); // 2,29
      ...
      if (/*Enemy->*/ Mode == 124)
        Enemy_add_fireball(Enemy->X + 12, Enemy->Y + (Enemy->Life > 0 ? 5 : 26),
                           enemy_fireball_speed, enemy_fireball_speed); // 5,26
    };
  } else {                      // left
    if (Player.Y <= Enemy->Y) { // up
                                //			Enemy->Face = -2;
      ...
      if (/*Enemy->*/ Mode == 124)
        Enemy_add_fireball(Enemy->X - 4, Enemy->Y + (Enemy->Life > 0 ? 2 : 29),
      ...
      if (/*Enemy->*/ Mode == 124)
        Enemy_add_fireball(Enemy->X - 4, Enemy->Y + (Enemy->Life > 0 ? 5 : 26),
                           -enemy_fireball_speed, enemy_fireball_speed); // 5,26
    };
  };
...
void Enemy_handler_4(enemy *Enemy) { // flying monster 1
  ...
  if (/*Enemy->*/ Jumping < 0) {
    if (Free_down(
            /*Enemy->*/ X,
            /*Enemy->*/ Y +
                /*Enemy->*/ Height) /*Enemy_free_down(Enemy)*/) { // fly down
      /*Enemy->*/ Y += 1;
    } else {
      /*Enemy->*/ Jumping = 0;
    };
...
void Enemy_handler_6(enemy *Enemy) { // turtle/beetle shell
  ...
  if (/*Enemy->*/ Mode <= 12) { // shaking before resurection
    ...
    if (/*Enemy->*/ Mode == 0) { // resurection
      if (Enemy->X > Player.X) { // allways walk against player after
                                 // resurection
        Enemy->Sprite =
            (/*Enemy->*/ Life == 1 ? turtle_right_sprite : beetle_left_sprite);
        Enemy->Face = -1;
      } else {
        Enemy->Sprite =
            (/*Enemy->*/ Life == 1 ? turtle_left_sprite : beetle_right_sprite);
        Enemy->Face = 1;
      };
...
void Enemy_handler_8(enemy *Enemy) { // flying turtle
  ...
  if ((/*Enemy->*/ Mode & 0b00001000)) { // dont turn before bottom reached
    if ((Enemy->Data0 == 0) && (/*Enemy->*/ Jumping > 0)) {
      if (Free_down(Enemy->X,
                    Enemy->Y + Enemy->Height) /*Enemy_free_down(Enemy)*/) {
        Enemy->Data0 = 1;
      };
    };
  };
...
void Enemy_handler_11(enemy *Enemy) { // circulating fireball
  ...
  // radius = 40
  static const char Motion[30] = {4,  4,  4,  4,  4,  4,  3,  3,  2,  3,
                                  2,  1,  1,  1,  0,  0,  -1, -1, -1, -2,
                                  -3, -2, -3, -3, -4, -4, -4, -4, -4, -4};
  // Initialization:
  // Data0 = 15
...
void Enemy_handler_17(enemy *Enemy) { // handles mad jumping/walking flower
  ...
  if (Player.X < Enemy->X) {
    /*Enemy->*/ Sprite =
        (Fg_plane.step % 2 ? mad_flower_c_left_spr : mad_flower_o_left_spr);
    /*Enemy->*/ Face = -1;
  } else {
    /*Enemy->*/ Sprite =
        (Fg_plane.step % 2 ? mad_flower_c_right_spr : mad_flower_o_right_spr);
    /*Enemy->*/ Face = 1;
  };
...
void Enemy_handler_21(
  ...
  static const char dx[20] = {48,  -24, 0,  20,  -40, 0,   40, -20, -16, 32,
                              -16, -12, 24, -24, 24,  -12, 0,  -8,  16,  -8};
  ...
  if (Enemy->Mode > 2) {
    ...
    if ((Enemy->Mode < 50)) {
      ...
      if (Enemy->Mode > 23) {
        if (Fg_plane.step % 2) {
          Enemy->Sprite =
              (Enemy->Face > 0 ? bomb_deto_right_spr : bomb_deto_left_spr);
          Enemy->Height2 = 14;
        } else {
          Enemy->Sprite =
              (Enemy->Face > 0 ? bomb_walk_right_spr : bomb_walk_left_spr);
          Enemy->Height2 = 16;
        }; // Fg_plane.step%2
...
char Enemy_move_x(enemy *Enemy) {
  ...
  if (/*Enemy->*/ Face > 0) { // right
    if (Free_right(Enemy->X, &Enemy->Y, Enemy->Height, /*Enemy->*/ Face) >
    ...
    if (/*Enemy->*/ Face < 0) { // left
      if (Free_left(Enemy->X, &Enemy->Y, Enemy->Height, /*Enemy->*/ Face) >
...
void Enemy_die_5(enemy *Enemy) {
  ...
  Add_shell(/*Enemy->X,Enemy->Y,*/ ((Player.X >= Enemy->X) ? -shell_speed
                                                           : shell_speed),
            Enemy->Life, Enemy);
  Score_anim(Enemy->X, Enemy->Y - 8, score_200);
...
void Enemy_die_10(enemy *Enemy) { // handles the teath of boomerang guy,
  ...
  Enemy->Die = (void *)Dummy_func_; // prevent this func to be executed twice,
                                    // destroying treasure
  Enemy_die_6(Enemy);
};
  ...
  for (C = 0; C < nr_of_enemyshots; C++) {
    if (Enemyshots[C].Mode) { // if existing
      ...
      if (/*(!Player.Immortal) &&*/ TestCollide162h_R(
          Player.X, Player.Y, Enemyshots[C].X, Enemyshots[C].Y, Player.Height,
          Enemyshots[C].Height,
          Mariosprites +
        ...
        if (((Enemyshots[C].Mode < 0) &&
             ((Player.Y + Player.Height - 8) <= Enemyshots[C].Y)) ||
            ((SavePlayer.Attribs & 0b10000000) && (Enemyshots[C].Mode < 0))) {
      ...
      if ((FgX > Enemyshots[C].X) || ((FgX + screen_width) < Enemyshots[C].X) ||
