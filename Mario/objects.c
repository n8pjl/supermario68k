...
/*
objects.c:
those variables and functions deals with all kind of objects in the game that doesn't come uner the categories 
Tiles, enemies, shots/fireballs, shells, player, etc.

An object can be: fragments of crushed bricks, dead enemy flying through the air, animations etc,
*/
...
void Handle_climbing_flower(object* Object){
	...
	if(Object->Data0<16){//coming out of box
		...
		if(Free_up(Object->X,Object->Y)){
			Object->Y -= 2;//grow
			//*( (char*)(Fg_plane.p.matrix+(((Object->Y+Object->Data0-1)/16))*Fg_plane.p.width+(Object->X/16)) )=ladder;
			...
			//*( (char*)(Fg_plane.p.matrix+(((Object->Y)/16))*Fg_plane.p.width+(Object->X/16)) )=ladder;
		};
