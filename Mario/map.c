...
inline void Handle_player_map(){
	...
	if(Map_statusbar==status){
		...
		if( (CX == 0) && (CY == 0) ){
			...
			if(Keystate.Right /*&& ((Player.MapX)<(Map_data.Border_right))*/ && ( (SavePlayer.L==0) || (SavePlayer.L==2) ) ){
				CX = 0;
				Player.Face = 1;
				if(!SavePlayer.IsOnMapBoat){//not in boat
					if( (CX = Free_right_map(SavePlayer.MapX,SavePlayer.MapY))==0 ){//set player motion
												
						for(C=0;C<Map_data.Nr_of_objects;C++){//check if boat in range
							if( (Map_objects[C].Mode==30) && (Map_objects[C].X==(SavePlayer.MapX+16)) && (Map_objects[C].Y==(SavePlayer.MapY)) ){
								CX = 16;//move to boat
						...
						if(Get_map_tile(SavePlayer.MapX+CX,SavePlayer.MapY)>=levels_low){
					...
					if( (Get_map_tile(SavePlayer.MapX+16,SavePlayer.MapY)==water_map) && (Map_data.Border_right>(SavePlayer.MapX+16)) ){//check if water
						CX = 16;
						Map_objects[(short)SavePlayer.IsOnMapBoat].MoveX = 16;//move boat along with player
					};
					if(Get_map_tile(SavePlayer.MapX+16,SavePlayer.MapY)==dock){//check if dock in range
						CX = 16;//move to dock
				...
				if(Keystate.Left /*&& (Player.MapX>Map_data.Border_left)*/ && ( (SavePlayer.L==0) || (SavePlayer.L==1) ) ){
					CX = 0;	
					Player.Face = -1;
					//Replace_by_road(4*16,5*16);//test case
					
					if(!SavePlayer.IsOnMapBoat){//not in boat
						if( (CX = Free_left_map(SavePlayer.MapX,SavePlayer.MapY))==0 ){//set player motion
							for(C=0;C<Map_data.Nr_of_objects;C++){//check if boat in range
								if( (Map_objects[C].Mode==30) && (Map_objects[C].X==(SavePlayer.MapX-16)) && (Map_objects[C].Y==(SavePlayer.MapY)) ){
									CX = -16;//move to boat
									SavePlayer.IsOnMapBoat = C;//"subscribe"  to this boat
						...
						if( (Get_map_tile(SavePlayer.MapX-16,SavePlayer.MapY)==water_map) && (Map_data.Border_left<SavePlayer.MapX)){//check if water
							CX = -16;
							Map_objects[(short)SavePlayer.IsOnMapBoat].MoveX = -16;//move boat along with player
						};
						if(Get_map_tile(SavePlayer.MapX-16,SavePlayer.MapY)==dock){//check if dock in range
					...
					if(Keystate.Up && (SavePlayer.MapY>0) && ( (SavePlayer.L==0) || (SavePlayer.L==4) ) ){
						CY = 0;			
						/*
						New_card_game();
						...
						*/
						...
						if(!SavePlayer.IsOnMapBoat){//not in boat
							if( (CY = Free_up_map(SavePlayer.MapX,SavePlayer.MapY))==0 ){//set player motion
								for(C=0;C<Map_data.Nr_of_objects;C++){//check if boat in range
									if( (Map_objects[C].Mode==30) && (Map_objects[C].X==(SavePlayer.MapX)) && (Map_objects[C].Y==(SavePlayer.MapY-16)) ){
										CY = -16;//move to boat
										SavePlayer.IsOnMapBoat = C;//"subscribe"  to this boat
									};
								};
								...
								if(Get_map_tile(SavePlayer.MapX,SavePlayer.MapY+CY)>=levels_low){
							...
							if( (Get_map_tile(SavePlayer.MapX,SavePlayer.MapY-16)==water_map) ){//check if water
								CY = -16;
								Map_objects[(short)SavePlayer.IsOnMapBoat].MoveY = -16;//move boat along with player
							};
							if(Get_map_tile(SavePlayer.MapX,SavePlayer.MapY-16)==dock){//check if dock in range
								CY = -16;
								SavePlayer.IsOnMapBoat = 0;//"unsubscribe" the current boat
						...
						if(Keystate.Down && (SavePlayer.MapY<(16*Map_data.Height-1)) && ( (SavePlayer.L==0) || (SavePlayer.L==3) ) ){
							CY = 0;
							Player.Face = -2;
							if(!SavePlayer.IsOnMapBoat){//not in boat
								if( (CY = Free_down_map(SavePlayer.MapX,SavePlayer.MapY))==0 ){//set player motion
									for(C=0;C<Map_data.Nr_of_objects;C++){//check if boat in range
										if( (Map_objects[C].Mode==30) && (Map_objects[C].X==(SavePlayer.MapX)) && (Map_objects[C].Y==(SavePlayer.MapY+16)) ){
											CY = 16;//move to boat
											SavePlayer.IsOnMapBoat = C;//"subscribe"  to this boat
		...
		if( Keystate.Jump && !Previous_keystate.Jump){
			...
			if( (Temp2>=levels_low) && (Temp2<=levels_high) ){
				...
				Load_level(Levelfilename,Get_map_tile(SavePlayer.MapX,SavePlayer.MapY)-levels_low);
				...
				if(Exit == 2){
					...
					if( (Temp2 == small_castle) || (Temp2 == small_castle_2) ){
						...
						Temp = (Map_data.Color==2?demolished_castle_dark:demolished_castle);//demolished_castle;
						Replace_map_tile( (Temp2 == small_castle?locked_door:locked_door_2) );
					}
					else{
						Temp = M_tile;
					};				
					...
					//check: Event? (Appearance of Card Game, money ship, hidden mushrom house)
...
//					CY = SavePlayer.PrevCompY - SavePlayer.MapY;
				...
				switch(Temp2){
					...
					case pipe:
						...
						for(C=0;C<Map_data.Nr_of_triggers;C++){
							...
							if( (Map_triggers[C].X == SavePlayer.MapX) && (Map_triggers[C].Y == SavePlayer.MapY) ){

								//Load_level("common",Map_triggers[C].LevelNr);
								if(Map_triggers[C].NewMap>=0){//the pipe wraps to another world (warp zone)
									//jump to another world
									//animate
									Levelsetdata.CurrentWorld = Map_triggers[C].NewMap;
									Exit = 3;
								}
								else{//the pipe wraps to another location in the current world
		...
		if(Keystate.Right && !Previous_keystate.Right && SavePlayer.Itemlist[Player.Curr_item+1] && Player.Curr_item<(itemlist_length-1)){
			Player.Curr_item++;
		};
		if(Keystate.Jump && !Previous_keystate.Jump){
			if(SavePlayer.Itemlist[(short)Player.Curr_item]){
				...
				switch(SavePlayer.Itemlist[(short)Player.Curr_item]){
					case 1:{//mushrom
						...
						if(SavePlayer.Life == 1){//only effective if small mario (if racoon mario or fire mario: don't do anything)
							SavePlayer.Life = 2;
...
//						Levelsetdata.CurrentWorld = Levelsetdata.Nr_of_files-2;
//						Exit = 100;
					...
					case 6:{//hammer
						for(C=0;C<=3;C++){
							...
							if(Get_map_tile(SavePlayer.MapX+Searchtable[C],SavePlayer.MapY+Searchtable[3-C])==rock){//rock?
								
								short X = SavePlayer.MapX+2*Searchtable[C];
								short Y = SavePlayer.MapY+2*Searchtable[3-C];
								if( (Get_map_tile(X,Y)<=walkable_high) && (X>0) && (X<(Map_data.Width*16)) && (Y>0) && (Y<(Map_data.Height*16)) ){//walkable behind the rock?
							
									Temp=(C<2?1:0);//decide which direction (x/y)=>which road to apply
								...
								Put_map_tile(SavePlayer.MapX+Searchtable[C],SavePlayer.MapY+Searchtable[3-C],Temp);
							};						
						};
						break;
					};
...
void Enter_enemy_ship(map_object *Ship){
	...
	*/
	SavePlayer.Attribs = SavePlayer.Attribs & 0b01110111;//disable star and p-wing
	Map_plane.p.force_update = 1;
	...
	if( (Exit == 1) ){//mario failed, 
		...
		if( Leveldata.Condition && (Ship->Mode == 1)){//ship and not anchored
			...
			C = ((Ship->X>>4) + Map_data.Width*(Ship->Y>>4) +1);//find array index in map (ship position)
			...
			while( (OldFgX!=Ship->X) || (OldFgY!=Ship->Y) ){
				...
				if( !(BoundsCollide2(Ship->X,Ship->Y,16,16,FgX,FgY,screen_height,screen_width)) ){//if ship outside visible part of screen
					Ship->X = OldFgX;
					Ship->Y = OldFgY;
				};				
...
//		FastFilledRect_Erase_R(dBufHPL_G,0,25,43,75);
		...
		//some text here...
		//DrawGrayStrExt2B(28,23,Texts+GameTextData.ThankU1,A_REPLACE,F_4x6,dBufHPD_G,dBufHPL_G);//"THANK YOU MARIO, BUT OUR"
		//DrawGrayStrExt2B(28,33,Texts+GameTextData.ThankU2,A_REPLACE,F_4x6,dBufHPD_G,dBufHPL_G);//"PRINCESS IS IN ANOTHER CASTLE!"
		//DrawGrayStrExt2B(28,43,Texts+GameTextData.ThankU3,A_REPLACE,F_4x6,dBufHPD_G,dBufHPL_G);//spare, in case of needed
...
//		DrawGrayStrExt2B(28,48
		...
		//Use three lines in case some translator comus up with a very long sentence....
...
//		GrayClipSprite16_SMASK_R(40,58,Height,Sprite,Sprite+Height,Sprite+2*Height,/*GrayDBufGetHiddenPlane(LIGHT_PLANE)*/dBufHPL_G,/*GrayDBufGetHiddenPlane(DARK_PLANE)*/dBufHPD_G);
		...
		DrawSprite16SMASKR(104,51,30,princess_spr,princess_spr+30,princess_spr+2*30);
...
inline void Handle_map_objects(){
	...
	for(C=0;C<Map_data.Nr_of_objects;C++){
		...
		if(Map_objects[C].Mode){
			...
			if( (BoundsCollide16(SavePlayer.MapX,SavePlayer.MapY,Map_objects[C].X,Map_objects[C].Y)) && (Map_objects[C].Mode<=20) )
				Player.Passified=1;
						
			
			if( (SavePlayer.MapX == (Map_objects[C].X) ) && (SavePlayer.MapY == Map_objects[C].Y) ){
...
unsigned char Get_map_tile(short X,short Y){
		...
		return *( (char*)(Map_plane.p.matrix+(Y>>4)*Map_plane.p.width+(X>>4)) );
	
}

void Put_map_tile(short X,short Y,unsigned char Tile){
	
	*( (char*)(Map_plane.p.matrix+(Y>>4)*Map_plane.p.width+(X>>4)) ) = Tile;
...
void Handle_map_monster(map_object* Monster){
	...
	if(Move_map_objects){
		...
		switch(Map_plane.step){//generates pseudo random direction for monster movement
		...
		unsigned char Temp = Get_map_tile(Monster->X+Monster->MoveX-Monster->Data0,Monster->Y+Monster->MoveY);
		if( !((Temp>=walkable_node_low) && (Temp<=walkable_node_high)) ){
			Monster->MoveX = Monster->MoveY = 0;//abort, if target tile is not a node, i.e. a path
//			Player.Passified=0;
//			Move_map_objects=0;
//			Player.Offset = 0;
		}
		...
		if(Monster->MoveX || Monster->MoveY){
			...
			if( (Monster->X>=(FgX-16)) && (Monster->X<=(FgX+screen_width)) && (Monster->Y>=(FgY-16)) && (Monster->Y<=(FgY+screen_height-16)) ){
...
void Enter_map_anim(){
	...
	short LX=SavePlayer.MapX,RX=SavePlayer.MapX+16,UY=SavePlayer.MapY,LY=SavePlayer.MapY+16;
...
void Enter_level_anim(){
	...
	while( (LX < RX) || (LY > HY) ){
		...
		GrayFastOutlineRect_R(GrayDBufGetActivePlane(LIGHT_PLANE),GrayDBufGetActivePlane(DARK_PLANE),LX,HY,RX,LY,COLOR_DARKGRAY);
	...
	//FastDrawLine_R(register unsigned char* plane asm("%a0"),register short x1 asm("%d0"),register short y1 asm("%d1"),register short x2 asm("%d2"),register short y2 asm("%d3"),short mode)
...
short Free_left_map(short X, short Y){
	...
	while( (Get_map_tile(X-16+DX,Y)<=levels_high) && ((X+DX)>Map_data.Border_left) ){
		DX -= 16;
...
short Free_right_map(short X, short Y){
	...
	while( (Get_map_tile(X+16+DX,Y)<=levels_high) && ((X+DX+16)<Map_data.Border_right) ){
		DX += 16;
		if(Get_map_tile(X+DX,Y)>=walkable_node_low){
			break;
		};
...
/*	short Set=-1;
	...
	/*if( (X==0) || (Y==0) || (X==(Map_data.Height)) || (Y==(Map_data.Width)) ){//if on the edge of the map...
		Put_map_tile(X,Y,);//no road, plain ground instead
	}*/
