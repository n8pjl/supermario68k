...
static void blit_glyph(uint8_t *plane, int16_t x, int16_t y, const uint8_t *g,
		       int16_t h, int16_t adv, int16_t attr)
{
...
{
	draw_str_plane(GrayDBufGetHiddenPlane(LIGHT_PLANE), x, y, s, attr,
		       F_6x8);
	draw_str_plane(GrayDBufGetHiddenPlane(DARK_PLANE), x, y, s, attr,
		       F_6x8);
}
		      ...
		      int16_t font, void *lightplane, void *darkplane)
{
	...
	if (attr & A_CENTERED)
		x = (screen_width - str_width(s, font)) / 2;
