    ...
    */
    ...
    dBufHPD = GrayDBufGetHiddenPlane(DARK_PLANE);*/
      ...
      */
			...
			DrawSprite16SMASKR(
				RollX_176(C * 44 + X1) + screen_offset_sg_x, 30,
				12, game_mushrom_l + 4 * 38 * Sequence[C],
				game_mushrom_l + 38 + 4 * 38 * Sequence[C],
				blank_sprite);
			DrawSprite16SMASKR(
				RollX_176(C * 44 + X1 + 16) +
					screen_offset_sg_x,
				30, 12, game_mushrom_r + 4 * 38 * Sequence[C],
			...
			DrawSprite16SMASKR(
				RollX_176(C * 44 + X2) + screen_offset_sg_x, 42,
				16, game_mushrom_l + 4 * 38 * Sequence[C] + 12,
				game_mushrom_l + 38 + 4 * 38 * Sequence[C] + 12,
				blank_sprite);
			DrawSprite16SMASKR(
				RollX_176(C * 44 + X2 + 16) +
					screen_offset_sg_x,
				42, 16,
				game_mushrom_r + 4 * 38 * Sequence[C] + 12,
			...
			DrawSprite16SMASKR(
				RollX_176(C * 44 + X3) + screen_offset_sg_x, 58,
				10, game_mushrom_l + 4 * 38 * Sequence[C] + 28,
				game_mushrom_l + 38 + 4 * 38 * Sequence[C] + 28,
				blank_sprite);
			DrawSprite16SMASKR(
				RollX_176(C * 44 + X3 + 16) +
					screen_offset_sg_x,
				58, 10,
				game_mushrom_r + 4 * 38 * Sequence[C] + 28,
		...
		GrayOutlineRect(41 + screen_offset_sg_x, 23,
				121 + screen_offset_sg_x, 77, COLOR_WHITE);
		GrayOutlineRect(40 + screen_offset_sg_x, 22,
				122 + screen_offset_sg_x, 78, COLOR_WHITE);
		...
		if (State == 5) { // match
			...
			if ((AnimCount / 16) % 2) {
				...
				DrawSprite16SMASKR(79 + screen_offset_sg_x, 44,
						   15, up_sprite,
						   up_sprite + 15,
						   blank_sprite);
				GrayClipSprite8_SMASK_R(
					67 + screen_offset_sg_x, 44, 15,
					Smallsprites + Sprite,
					Smallsprites + Sprite + 15,
					(uint8_t *)blank_sprite,
		...
		if (Keystate.run && !(Previous_keystate.run) && !(Stopping)) {
			...
			case 1:
				...
				// f� pixler for langt til h�yre
				Stopping = -(
					176 +
					(X2 -
					 Find_stop_pos(
						 X2))); //(Find_stop_pos(X2) - X2) - 176;
...
{ // used in cardgame. Slower, but gives size optimization.
    ...
    */
    ...
    dBufHPD = GrayDBufGetHiddenPlane(DARK_PLANE);*/
		...
		FastFilledRect_Draw_R(
			/*GrayDBufGetHiddenPlane(LIGHT_PLANE)*/ dBufHPL_G, 0, 0,
			239, 127); //,0,25,43,75);
		...
		for (C = 0; C < 6; C++) {
			for (D = 0; D < 3; D++) {
				...
				GrayOutlineRect(
					4 + screen_offset_sg_x + 26 * C,
					3 + screen_offset_sg_y + 32 * D,
					23 + screen_offset_sg_x + 26 * C,
					30 + screen_offset_sg_y + 32 * D,
					COLOR_BLACK);
				GrayOutlineRect(
					24 + screen_offset_sg_x + 26 * C,
					6 + screen_offset_sg_y + 32 * D,
					25 + screen_offset_sg_x + 26 * C,
					32 + screen_offset_sg_y + 32 * D,
					COLOR_DARKGRAY);
				GrayOutlineRect(
					7 + screen_offset_sg_x + 26 * C,
				...
				SetPixel(
					5 + 26 * C + screen_offset_sg_x,
					4 + screen_offset_sg_y +
						32 * D); // same as above, but smaller
				...
				if ((CardGame[C + 6 * D] < 0)) {
					...
					GrayClipSprite16_SMASK_R(
						...
						6 + screen_offset_sg_y + 32 * D,
						22, card_top, card_top + 22,
						blank_sprite, dBufHPD_G,
						dBufHPL_G);
					...
					if ((!Flash) ||
						...
						if (CardGame[C + 6 * D] <= 2) {
							...
							DrawSprite16SMASKR(
								6 + screen_offset_sg_x +
									26 * C,
								9 + screen_offset_sg_y +
									32 * D,
								16, Sprite,
								Sprite + 16,
								Sprite + 32);
		...
		if (!Wait) {
			...
			if (Keystate.jump) {
				if (CardGame[CX + 6 * CY] < 0) { // unturned
					...
					if (PX >=
					    0) { // already turned a card (another card is open)
