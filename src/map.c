...
void Handle_player_map()
{
	...
	if (Keystate.run &&
	    !Previous_keystate.run) { // toggle map statusbar state
	...
	if (Map_statusbar == status) {
			...
			if (Keystate.right /*&& ((Player.MapX)<(Map_data.Border_right))*/
				...
				if (!SavePlayer.IsOnMapBoat) { // not in boat
					...
					if ((Get_map_tile(SavePlayer.MapX + 16,
					    ...
					    (Map_data.Border_right >
					     (SavePlayer.MapX +
					      16))) { // check if water
						CX = 16;
						Map_objects[(int16_t)SavePlayer
								    .IsOnMapBoat]
							.MoveX =
					...
					if (Get_map_tile(SavePlayer.MapX + 16,
						...
						CX = 16; // move to dock
						SavePlayer.IsOnMapBoat =
							0; //"unsubscribe" the current boat
						SavePlayer.PrevCompX =
							SavePlayer.MapX + 16;
				...
				if (Keystate.left /*&& (Player.MapX>Map_data.Border_left)*/
					...
					if (!SavePlayer.IsOnMapBoat) { // not in boat
						...
						if ((Get_map_tile(
							     ...
							     SavePlayer.MapY) ==
						     water_map) &&
						    (Map_data.Border_left <
						     SavePlayer.MapX)) { // check if water
							CX = -16;
							Map_objects
								[(int16_t)SavePlayer
									 .IsOnMapBoat]
		...
		if (Keystate.jump && !Previous_keystate.jump) {
			...
			if ((Temp2 >= levels_low) && (Temp2 <= levels_high)) {
				...
				if (Exit == 2) {
                  ...
                  Exit = 255;
					...
					if ((Temp2 == small_castle) ||
					    (Temp2 == small_castle_2)) {
						// Temp = demolished_castle
						Temp = (Map_data.Color == 2 ?
								demolished_castle_dark :
            ...
            */
...
#ifndef debug
					if (Exit == 99 ||
					    Exit == 9) // ( gives debug mode!)
						return;
#endif
#ifdef debug
					if (Exit ==
					    99 /*|| Exit==9*/) // ( gives debug mode!)
						return;
#endif
					Player.Offset = 1;
				...
				case pipe:
					...
					for (C = 0; C < Map_data.Nr_of_triggers;
					     C++) {
						if ((Map_triggers[C].X ==
						     SavePlayer.MapX) &&
						    (Map_triggers[C].Y ==
		...
		if (Keystate.jump && !Previous_keystate.jump) {
			if (SavePlayer.Itemlist[(int16_t)Player.Curr_item]) {
				// collect item
				int16_t Remove = 1;
				switch (SavePlayer.Itemlist
						[(int16_t)Player.Curr_item]) {
				case 1: { // mushrom
				...
				case 2: { // fire flower
					...
					SavePlayer.Attribs =
						SavePlayer.Attribs |
						0b01000000; // add fireball ability
					SavePlayer.Attribs =
						SavePlayer.Attribs &
					...
					for (C = 0; C <= 3; C++) {
						if (Get_map_tile(
							...
							if ((Get_map_tile(X,
							    ...
							    (Y <
							     (Map_data.Height *
							      16))) { // walkable behind the rock?

								Temp = (C < 2 ? 1 :
										0); // decide which direction (x/y)=>which
								// road to apply
	...
	if (Exit == 1) { // mario failed,
		...
		if (Leveldata.Condition &&
			...
			while ((OldFgX != Ship->X) || (OldFgY != Ship->Y)) {
				if (OldFgX != Ship->X)
					Ship->X += (OldFgX > Ship->X ? 2 :
								       -2); // 1
				if (OldFgY != Ship->Y)
					Ship->Y += (OldFgY > Ship->Y ? 2 :
								       -2); // 1
				if (!(BoundsCollide2(
					    Ship->X, Ship->Y, 16, 16, FgX, FgY,
					    screen_height,
...
void Handle_map_objects()
{
	...
	for (C = 0; C < Map_data.Nr_of_objects; C++) {
		if (Map_objects[C].Mode) {
			...
			if ((BoundsCollide16(SavePlayer.MapX, SavePlayer.MapY,
					     Map_objects[C].X,
					     Map_objects[C].Y)) &&
			...
			if ((SavePlayer.MapX == (Map_objects[C].X)) &&
				...
				if (Map_objects[C].Mode <= 20)
					...
					case 10: // Hidden mushrom house Exit = 0;
						Enter_mushrom_house(
							Map_objects[C].Treasure);
						Map_objects[C].Mode = 0;
						break;
					case 12: // Money ship
						Load_level(Commonfilename,
							   Map_objects[C].Mode -
								   1);
...
{
	return *((char *)(Map_plane.p.matrix + (Y >> 4) * Map_plane.p.width +
			  (X >> 4)));
}

void Put_map_tile(int16_t X, int16_t Y, uint8_t Tile)
{
	*((char *)(Map_plane.p.matrix + (Y >> 4) * Map_plane.p.width +
		   (X >> 4))) = Tile;
}
...
void Handle_map_monster(struct map_object *Monster)
{
	...
	if (Move_map_objects) {
		...
		case 0: {
			if ((Monster->MoveX = Free_right_map(
				     /*Monster->*/ X, /*Monster->*/ Y))) {
		...
		if (!((Temp >= walkable_node_low) &&
		      (Temp <= walkable_node_high))) {
...
void Enter_level_anim()
{
	...
	while ((LX < RX) || (LY > HY)) {
		GrayFastOutlineRect_R(GrayDBufGetActivePlane(LIGHT_PLANE),
				      GrayDBufGetActivePlane(DARK_PLANE), LX,
				      HY, RX, LY, COLOR_DARKGRAY);
...
void Replace_map_tile(uint8_t Tile)
{
	...
	for (C = 0; C < Map_data.Height * Map_data.Width; C++) {
		if (*((uint8_t *)(Map_plane.p.matrix + C)) == Tile) {
			Replace_by_road((C % Map_data.Width) * 16,
					(C / Map_data.Width) * 16);
		}
		/*else{
  ...
  instead
	...
	if (Get_map_tile(X - 16, Y) <= walkable_high) { //-
		...
		if (Get_map_tile(X, Y - 16) <= walkable_high) { //|
			if (Get_map_tile(X + 16, Y) <=
			    walkable_high) { //|_ found
				// put tile : 16
