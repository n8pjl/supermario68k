	...
	for (C = 0; C < max_nr_of_shells; C++) {
		...
		if (/*Shells[C].*/ Shell->Active) { // test if Active(existing)
			...
			if ((/*Shells[C].*/ Shell->X > (FgX - 64)) &&
			    (/*Shells[C].*/ Shell->X <
			     (FgX + BIG_VSCREEN_WIDTH + 64)) &&
			    (/*Shells[C].*/ Shell->Y > (FgY - 64)) &&
			    (/*Shells[C].*/ Shell->Y <
			     (FgX + BIG_VSCREEN_HEIGHT +
			      64))) { // in active area ?

				if (/*Shells[C].*/ Shell->Dir >
				    0) { // dir = right ?
					...
					if (Dist <= 0) { // not free right
						...
						if ((Temp >=
							...
							Interactive_tile_handlers[Temp -
						...
						if ((/*Shells[C].*/ Shell->X) >
						    16 * (int16_t)Fg_plane.p
								    .width) // test if outside level (right)
							/*Shells[C].*/ Shell
								->Active =
								0; // kill shell
					};
