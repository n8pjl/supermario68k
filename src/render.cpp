...
int16_t scroll_limit_right = 130;
int16_t scroll_limit_up = 49;
int16_t scroll_limit_down = 79;
int16_t screen_width = 240;
int16_t screen_height = 128;
int16_t statusbar_offset = 3600;
int16_t screen_offset_sg_x = 40;
int16_t screen_offset_sg_y = 14;

void init_calc_screen_constants(bool ti89_mode)
{
	if (ti89_mode) {
		scroll_limit_left = 70;
		scroll_limit_right = 90;
		scroll_limit_up = 35;
		scroll_limit_down = 65;
		screen_width = 160;
		screen_height = 100;
		statusbar_offset = 2760;
		screen_offset_sg_x = 0;
		screen_offset_sg_y = 0;
	} else {
		scroll_limit_left = 110;
		scroll_limit_right = 130;
		scroll_limit_up = 49;
		scroll_limit_down = 79;
		screen_width = 240;
		screen_height = 128;
		statusbar_offset = 3600;
		screen_offset_sg_x = 40;
		screen_offset_sg_y = 14;
	}
}
		...
		int16_t H2)
{
	...
	GrayClipSprite16_SMASK_R(
		X - FgX, Y - FgY, H, sprt0, sprt0 + H2, sprt0 + H2 + H2,
  ...
  */
	...
	for (Curr_enemy = 0; Curr_enemy < Leveldata.Nr_of_enemies;
		...
		if (Enemies[Curr_enemy].Active &&
			...
			if (Enemies[Curr_enemy].Life > 0) {
				...
				// //USING CLIPPED SPRITE TO AVOID DRAVING OUTSIDE GFX MEM
				DrawSprite(Enemies[Curr_enemy].X,
	...
	for (C = 0; C < nr_of_player_smallshots; C++) {
		if (Player_smallshots[C].Mode)
			...
			DrawSprite(Player_smallshots[C].X,
				   Player_smallshots[C].Y, 8,
		...
		if (Flying_platforms[C].Active) {
			for (D = 0; D <= (Flying_platforms[C].Width - 1); D++) {
				...
				if (Flying_platforms[C].Sprite ==
				    60000) { // TEMP, THIS WILL CHANGE!!
					DrawSprite(Flying_platforms[C].X,
						   Flying_platforms[C].Y, 16,
						   falling_block_spr, 16);
				} else {
					GrayClipISprite16_RPLC_R(
						Flying_platforms[C].X - FgX +
							(D) * 16,
...
void Adjust_renderpoint()
{ // adjusts the FG renderpoint
	...
	if (((Player.Y + Player.Height) > (FgY + scroll_limit_down)) &&
		...
		FgY = FgY +
		      ((Player.Y + Player.Height) - (FgY + scroll_limit_down));
	} else {
...
void Draw_brick_fragments(struct object *Object)
{
  ...
  */
	...
	GrayClipSprite8_SMASK_R(/*(Object->X-Object->Active)-FgX*/ X1,
				/*(Object->Y-8*Object->Data0)-FgY*/ Y1, 8,
...
void Draw_collided_shells(struct object *Object)
{
	...
	if (Object->Active >
	    28) // with speed 4 in y dir this nr ensures that none is
	...
	DrawSpriteUpsideDown(Object->X, Object->Y, 16,
...
void Draw_climbing_flower(struct object *Object)
{
	if (Object->Data0 < 16) { // coming out of box
		...
		if (Free_up(Object->X, Object->Y)) {
			...
			Put_tile(Object->X, Object->Y + Object->Data0 - 1,
				 ladder);
			if (!(Object->Y % 16)) {
				// if(!(Object->Y&0x000f)){
				Update_FG =
...
{ // one dummy call neccesarry with all the
	...
	if (Update_frame_counter) {
		...
		DrawGrayStrExt2B(145, 1, "        ", A_REPLACE, F_4x6,
	...
	if (OldScore != SavePlayer.Score) {
		...
		DrawGrayStrExt2B(screen_width - (32), 1, String, A_REPLACE,
				 F_4x6, Statusbar, Statusbar + 240);
	}
#endif
	...
	for (C = 0; C < 4; C++) {
		...
		GrayClipSprite8_SMASK_R(
			78 + 8 * C, 0, 8,
			(Player.Runcount >= ((flycondition / 4) * (C + 1)) ?
				 triangle_f_spr :
				 triangle_e_spr),
			(Player.Runcount >= ((flycondition / 4) * (C + 1)) ?
				 triangle_f_spr :
	...
	memcpy(/*GrayDBufGetHiddenPlane(LIGHT_PLANE)*/ dBufHPL_G +
		       statusbar_offset,
	       Statusbar, 240); // 2760
...
void Draw_player()
{
	...
	if ((SavePlayer.Attribs & 0b00100000) &&
		...
		C = 9 *
		    Marioanimtab[4 + SavePlayer.Spritebase]
				[SavePlayer.Spritenr]; // 9*Marioanimtab+(4+Player.Spritebase)*8+Player.Spritenr;

		if (Player.RacoonTaleAnim) {
			C = 9 * Marioanimtab[4 + SavePlayer.Spritebase]
					    [(Fg_plane.step % 2 ? 0 : 2)];
		...
		if (SavePlayer.Spritenr != 9) {
			GrayClipSprite16_SMASK_R(
				...
				Player.Y - FgY +
					(SavePlayer.Spritenr == 10 ? 13 : 17),
				9, racoon_tale_spr + 2 * C,
				racoon_tale_spr + 9 + 2 * C,
				racoon_tale_msk + C,
	...
	GrayClipSprite16_SMASKBLIT_R(
		...
		Mariomasks +
			Marioanimtab[SavePlayer.Maskbase][SavePlayer.Spritenr] +
			Player.SpriteOffset,
	...
	if (SavePlayer.Attribs & 0b00100000) { // draw racoon suit cap
		...
		if (Player.SpriteOffset < 5) {
			...
			GrayClipSprite16_SMASKBLIT_R(
				Player.X - FgX,
				Player.Y - FgY +
					(SavePlayer.Spritenr == 4 ? 4 : -1),
				5 - Player.SpriteOffset,
				racoon_cap_spr + 2 * C + Player.SpriteOffset,
...
void Render_map()
{
  ...
  */
	...
	if (Levelsetdata.CurrentWorld ==
	    Levelsetdata
		    .Commonfile) { // Warp zone: Find pipe pos and draw numbers
		int16_t C;
		for (C = 0; C < Map_data.Nr_of_triggers; C++) {
			int16_t NewMap =
				Map_triggers[C].NewMap + 1; // + 26   + C;;
			char String[3];
      ...
      short Y = Map_triggers[C].Y-FgY-8;*/
			...
			if (((Map_triggers[C].X + 1) >= FgX) &&
				...
				DrawString(Map_triggers[C].X - FgX + 1,
					   Map_triggers[C].Y - FgY - 8, String,
					   A_NORMAL, F_6x8);
			}
		}
	}
	...
	DrawSprite16SMASKR(SavePlayer.MapX - FgX, SavePlayer.MapY - FgY, 16,
			   Sprite, Sprite + 16, Mask);
  ...
  */
	...
	if (Map_statusbar == status) {
		for (C = 1; C <= 3; C++) {
			...
			GrayOutlineRect(screen_width - 18 * C,
					screen_height - 16,
		...
		if (SavePlayer.Attribs & 0b00100000) { // draw racoon suit cap
			...
			DrawSprite16SMASKR(2, screen_height - 16, 5,
					   racoon_cap_spr + 2 * C,
					   racoon_cap_spr + 5 + 2 * C,
					   racoon_cap_msk + C);
		}
		...
		GrayClipSprite16_SMASK_R(
			...
			Mariosprites +
				2 * Marioanimtab[SavePlayer.Spritebase /*2*/][9],
			Mariosprites +
				2 * Marioanimtab[SavePlayer.Spritebase /*2*/][9] +
				/*27*/ Player.Height2,
			Mariomasks +
				Marioanimtab[SavePlayer.Spritebase /*2*/][9],
			GrayDBufGetHiddenPlane(Left /*LIGHT_PLANE*/),
			GrayDBufGetHiddenPlane(Right /*DARK_PLANE*/));

		GrayClipISprite16_RPLC_R(
			50, screen_height - 16, 16,
