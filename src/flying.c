	...
	if ((++Platform->Active) == 3) {
        ...
        */
				...
				if (Platform->Data2 > 0) {
					...
					// Set_player_attribs_right(P_attribs);
					P_attribs[5] = Free_right(
						Player.X, &Player.Y,
						Player.Height, Platform->Data2);
				} else {
					// New: V 1.04 Changed from using Set_attribs func to Free_dir func.
					//  This Fixed bug where player died on contact with flying platforms
					//  when he was positioned on slope tiles Mario is now pushed to the
...
void Platform_handler_3(struct flying_platform *Platform)
{
	...
	if ((Platform->Data1) < 40) { // shake time
		...
		// Platform->Active++;
		// shake
		if (BoundsCollide2(Player.X, Player.Y, Player.Height, 15,
				   Platform->X, Platform->Y - 4, 16,
				   Platform->Width * 16)) {
		...
		Put_tile(Platform->X, Platform->Y,
			 0); // delete tile, start to fall
