...
int16_t Load_level(char *Levelfile, int16_t Level_nr)
{
	...
	SavePlayer.Attribs = SavePlayer.Attribs &
			     0b11101111; // turn of "draw mario before fg"
...
Leveldata.Bg_width = Bg_plane.width = Bg_data.Width;
  ...
  */
	...
	Enemies =
		(struct enemy *)(Level + Leveldata.Height * Leveldata.Width +
				 ((Leveldata.Height * Leveldata.Width) %
    ...
    */  		//end of "handler 1 enemies"
		...
		if ((Model >= 31) &&
		    (Model <=
		     42)) { // common for all enemies using "boomerang guy" sprite
    ...
    break;
		...
		case lavaball:
			...
			if (/*Enemies[Nr].*/ Face & 0b00010000)
				Enemies[Nr].Y += 8;
			/*Enemies[Nr].*/ Face =
				((/*Enemies[Nr].*/ Face & 0b00001111) - 8);
			/*Enemies[Nr].*/ Life =
				(/*Enemies[Nr].*/ Face > 0 ? 1 : -1);
...
// 					Enemies[Nr].Y -= 2;
	...
	memcpy(Triggers, Raw,
	       Leveldata.Nr_of_triggers *
		       sizeof(struct trigger)); // copy triggers
  ...
  */
	...
	while (Nr < Leveldata.Boss) {
		...
		case bowser: // boss_1:
			...
			BossG->Handler =
				(void (*)(void *))Bowser_handler; // Dummy_boss;
				 ...
				 uint8_t D0D1)
{
	...
	if (!(Model % 2)) {
		Enemies[Nr].Data0 = Enemies[Nr].Data1 =
			(D0D1 * 16); // common for all 1-20
		char Active;
		if (Enemies[Nr].Data0 ==
		    0) { // if zero: Mode 3: walk to the end and turn
			/*Enemies[Nr].*/ Active = 3;
		} else {
			/*Enemies[Nr].*/ Active = 2;
		}
...
static void Swap_map_payload()
{
	...
	for (C = 0; C < Map_data.Nr_of_objects - 3; C++) {
		...
		Map_objects[C].Handler = (void (*)(void *))be32(
			(uint32_t)Map_objects[C].Handler);
		Map_objects[C].Sprite =
			(uint16_t *)be32((uint32_t)Map_objects[C].Sprite);
		Map_objects[C].Mask =
			(uint16_t *)be32((uint32_t)Map_objects[C].Mask);
	}
}
  ...
  Exit = 3;
	...
	for (C = 0; C < Map_data.Nr_of_objects; C++) {
		if (Map_objects[C].Mode) {
			Map_objects[C].Sprite =
				Sprites + (int32_t)Map_objects[C].Sprite;
