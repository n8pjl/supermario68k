...
inline void Handle_flying_platforms() {
  ...
  for (C = 0; C < Leveldata.Nr_of_flying_platforms; C++) {
    ...
    if (Flying_platforms[C].Active) {
      ...
      if ((Flying_platforms[C].X > (Leveldata.Width * 16)) ||
          ((Flying_platforms[C].X + Flying_platforms[C].Width * 16) < 0) ||
...
void Platform_handler_1(struct flying_platform *Platform) {
  ...
  if ((++Platform->Active) == 3) {
    ...
    // New V 1.02: Corrected FgX to FgY in the line below. Fixed a bug where
    // some flying platforms dissappeared too early New V 1.04: Added &&
    // (Platform->Data3!=0): Preventing killing platforms that aren't moving
    if ((Platform->Data0 == 0) && (Platform->Data3 != 0) &&
        (Platform->Y >
         (FgY + screen_height +
          32))) {           // if falling with no return and outside active area
      Platform->Active = 0; // kill platform
    }

    if (BoundsCollide2(Player.X, Player.Y, Player.Height - 1, 15, Platform->X,
                       Platform->Y, 16, Platform->Width * 16)) {
      ...
      if (Platform->Data2) {
        ...
        */
        ...
        if (Platform->Data2 > 0) {
          ...
          //  This Fixed bug where player died on contact with flying platforms
          //  when he was positioned on slope tiles Mario is now pushed to the
          //  side and upwards instead of dieing when he is positioned on slope
          //  tiles

          // Set_player_attribs_right(P_attribs);
          P_attribs[5] =
              Free_right(Player.X, &Player.Y, Player.Height, Platform->Data2);
        } else {
          // New: V 1.04 Changed from using Set_attribs func to Free_dir func.
          //  This Fixed bug where player died on contact with flying platforms
          //  when he was positioned on slope tiles Mario is now pushed to the
          ...
          // Set_player_attribs_left(P_attribs);
          P_attribs[5] =
              Free_left(Player.X, &Player.Y, Player.Height, Platform->Data2);
        }
    ...
    if ((Platform->Data0 &&
         ((Platform->Data1 == 0) || (Platform->Data1 == Platform->Data0)))) {
...
void Platform_handler_3(struct flying_platform *Platform) {
  ...
  if ((Platform->Data1) < 40) { // shake time
    ...
    // Platform->Active++;
    // shake
    if (BoundsCollide2(Player.X, Player.Y, Player.Height, 15, Platform->X,
                       Platform->Y - 4, 16, Platform->Width * 16)) {
