...
int16_t doMenu(char *Menudata, int16_t Nr_of_options) {
  ...
  while (1) {
    ...
    DrawString(24, screen_height - 6, "http://tifreakware.net/lachprog/",
               A_REPLACE, F_4x6);
    //
    Lower = Upper + nr_of_visible_options -
            1; // min(Upper + nr_of_visible_options-1,Upper + Nr_of_options-1);
    C = 0;
    while (C < Nr_of_options) { // draw menu options
      ...
      if ((C >= Upper) && (C <= Lower)) {
        // GrayDrawStrExt(0,30+10*C,(Menudata+Next),A_REPLACE |
        // A_CENTERED,F_6x8);
        ...
        DrawString(30, 25 + 15 * (C - Upper), (Menudata + Next),
...
int16_t Menus() {
  ...
  if (C > 1) { // more than one levelset detected
    ...
    StringCopy(Levelsetfile,
               TempBuffer /*Fg_plane.p.big_vscreen*/ + 512 + (Res - 1) * 9);
  ...
  while (!Exit) {
    ...
    */
    while (Menu == load) {
      ...
      //			C = Offset =
      // StringCopy(TempBuffer/*Fg_plane.p.big_vscreen*/,Texts+GameTextData.Load);

      Paste_saveslot_text(TempBuffer /*Fg_plane.p.big_vscreen*/,
                          Texts + GameTextData.load_menu); // Offset);
