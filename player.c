...
void (*Interactive_tile_handlers[47])(int16_t X, int16_t Y) = {
    ...
    Bonus_box_coins, //
    Bonus_box_coins, Bonus_box_powerup_2, Bonus_box_pow, Bonus_box_powerup_1,
    Bonus_box_1up,                                      //
...
void Set_player_attribs_right(
  ...
  // right
  Attribs[0] =
      15 - ((Player.X + 15 + 15) & 0x000f); // same as above, but optimized
  Attribs[1] = Get_tile(Player.X + 15 + 16,
                        Player.Y + Player.Height - 1); // next tile right low
  Attribs[2] =
      Get_tile(Player.X + 15 + 16, Player.Y + 15);     // next tile right middle
  Attribs[3] = Get_tile(Player.X + 15 + 16, Player.Y); // next tile right high
  Attribs[4] =
      Get_tile(Player.X + 15, Player.Y + Player.Height - 1);    // 15   ,  -1
...
void Set_player_attribs_left(uint8_t *Attribs) {
  ...
  Attribs[1] = Get_tile(Player.X - 16,
  ...
  for (C = 0; C < Leveldata.Nr_of_flying_platforms; C++) {
    ...
    if (Flying_platforms[C].Active &&
      ...
      Temp2 = ((Player.X) -
               (Flying_platforms[C].X + Flying_platforms[C].Width * 16)); //-1
...
void Set_player_attribs_up(uint8_t *Attribs) {
  ...
  for (C = 0; C < Leveldata.Nr_of_flying_platforms; C++) {
    ...
    if (Flying_platforms[C].Active &&
        ...
        (Player.X <=
         (Flying_platforms[C].X + Flying_platforms[C].Width * 16))) {
...
void Set_player_attribs_down(uint8_t *Attribs) {
  ...
  Attribs[1] =
      Get_tile(Player.X, Player.Y + Player.Height + 15); // next tile down left
  Attribs[2] = Get_tile(Player.X + 15,
  ...
  if (Player.Y <
      (-Player.Height + 1)) { // adjust distance to next tile down when player
...
inline void Handleplayer() {
  ...
  if (Player.Racoonspin) {
    ...
    for (D = 0; D <= 9; D += 9) {
      for (C = 0; C <= 6; C += 6) {
        Temp = Get_tile(Player.X + (Player.Face == 1 ? -9 : 15) + D,
                        Player.Y + 17 + C);
        ...
        if ((Temp >= solid_interactive_bounch_low) &&
          ...
          Interactive_tile_handlers[Temp - solid_interactive_low](
              Player.X + (Player.Face == 1 ? -9 : 15) + D, Player.Y + 17 + C);
      };
    };
  };
  ...
  if (Player.Immortal) {
    ...
    if (!(--Player.Immortal)) {
      SavePlayer.Attribs =
          SavePlayer.Attribs & 0b01111111; // terminate super mode/star mode
    };
  };
  ...
  if (Keystate.Right || (Player.Xoffset > 0)) {
    ...
    if (Player.X < (Leveldata.Border_right - 16)) { // not at end
      ...
      if (Attribs[5] >= Player.Walkspeed) { // enough free space to move with
        ...
        if (Attribs[5] >= Player.Walkspeed2) {
          ...
          if ((Temp >= solid_interactive_left_right_low) &&
              (Temp <= solid_interactive_right_high))
            Interactive_tile_handlers[Temp - solid_interactive_low](
          ...
          if ((Temp >= solid_interactive_left_right_low) &&
              (Temp <= solid_interactive_right_high))
            Interactive_tile_handlers[Temp - solid_interactive_low](
                Player.X + 16, Player.Y + Player.Height - 1);
        };
      };
    };
    ...
    if (Player.X >=
      ...
      if (Attribs[5] >= Player.Walkspeed) { // enough free space
        ...
        if (Attribs[5] >= Player.Walkspeed2) {
          ...
          if ((Temp >= solid_interactive_left_low) &&
              (Temp <= solid_interactive_left_right_high))
            Interactive_tile_handlers[Temp - solid_interactive_low](
                Player.X - 1, Player.Y);
    ...
    if (Attribs[5] > 0) {         // free up?
      ...
      if ((!(Player.SlopeAbove)) && (Attribs[1] >= solid_low) &&
        ...
        if ((!(Player.SlopeAbove)) && (Attribs[1] < solid_low) &&
          ...
          if ((Temp >= solid_interactive_up_low) &&
              (Temp <= solid_interactive_up_down_high)) {
            Interactive_tile_handlers[Temp - solid_interactive_low](
                Player.X, Player.Y - 1);
    ...
    if ((Player.IsJumping >= (3 * Temp)) || (Player.IsFalling >= (3 * Temp))) {
      ...
      if (((Player.IsJumping < (3 * Temp)) &&
        ...
        if (((Player.IsJumping < (2 * Temp)) &&
            ...
            ((Player.IsFalling >= Temp) && (Player.IsFalling < (2 * Temp)))) {
          Player.Jumpspeed = 4;
          Player.Fallspeed = 4;
        } else {
  ...
  */
  ...
  SavePlayer.Attribs =
      SavePlayer.Attribs & 0b11101111; // draw player before fg OFF
  ...
  for (C = 0; C <= 15; C += 15) {
    ...
    if ((Temp >= non_solid_interactive_low) &&
      ...
      Interactive_non_solid_tile_handlers[Temp - non_solid_interactive_low](
          Player.X + C, Player.Y);
    ...
    if ((Temp >= non_solid_bonus_low) && (Temp <= non_solid_bonus_high))
      Player_collect_bonus(Player.X + C,
                           Player.Y + Player.Height - 1); // collect bonus
    if ((Temp >= non_solid_interactive_low) &&
        (Temp <= non_solid_interactive_high))
      Interactive_non_solid_tile_handlers[Temp - non_solid_interactive_low](
...
void Player_die_hard() {
  ...
  while (Player.Y < FgY + screen_height) { // die animation: jump up and fall
                                           // down, out of screen
...
uint8_t Get_tile(int16_t X, int16_t Y) {
  ...
  if ((Y >= 0) && (Y < (Leveldata.Height * 16))) {
    ...
    return *(
        (char *)(Fg_plane.p.matrix + (Y >> 4) * Fg_plane.p.width + (X >> 4)));
  };
